...
1) **success_rate** (Відсоток успішних транзакцій)
	**Пояснення:**
	Найважливіша ознака. Шахраї працюють з купленими базами крадених карток. Більшість таких карток вже заблоковані або не мають коштів, тому відсоток успішних оплат у фродерів аномально низький.
    **Вплив (SHAP):**
...
2) **avg_hours_from_reg** (Середній час від реєстрації до транзакції)
	**Пояснення:**
	Velocity-метрика. Звичайний користувач часто вивчає платформу, перш ніж щось купити. Шахраї або боти діють миттєво - транзакції починаються в перші хвилини або години після створення профілю.
    **Вплив (SHAP):**
	Чим менший час пройшов від реєстрації (сині точки), тим вищий ризик шахрайства.

3) **n_unique_card_holders** (Кількість унікальних імен власників)
    **Пояснення:**
	Легітимний користувач зазвичай використовує власні картки, де ім'я збігається. Наявність 3-5 різних імен власників на одному акаунті - аномалія.
    **Вплив (SHAP):**
...
4) **n_unique_cards** (Кількість унікальних карток)
	**Пояснення:**
	Аналогічно до попереднього пункту. Пересічний клієнт за статистикою датасета використовує 1-2 картки. Спроба прив'язати багато різних карток - аномальна поведінка.
    **Вплив (SHAP):**
	Велике значення ознаки підвищує ризик фроду.

5) **err_insufficient_funds_rate** (Частка помилок "insufficient funds")
    **Пояснення:**
	Кардери не знають балансу купленої картки. Тому помилка "insufficient funds" є частою при спробі зняти кошти.
    **Вплив (SHAP):**
	Висока частота таких помилок є стійким маркером шахрайської поведінки.
